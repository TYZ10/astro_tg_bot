...
class Predictions(BasicBotOperation):
    ...
    async def predictions_2_aspect(
        ...
        if period == "day":
            text = self.text.format("на день")
        elif period == "month":
            text = self.text.format("на месяц")
        else:
            text = self.text.format("на год")
